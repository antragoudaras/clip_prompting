...
class DeepPromptCLIP(nn.Module):
    ...
    def __init__(self, args, dataset, template="This is a photo of {}"):
        ...
        self.deep_prompt = nn.Parameter(torch.randn(args.prompt_num, self.clip_model.visual.transformer.width, dtype=self.clip_model.dtype), requires_grad=True)
        #######################
        # END OF YOUR CODE    #
        #######################
    ...
    def custom_encode_image(self, x):
        ...
        for i, transformer_block in enumerate(image_encoder.transformer.resblocks):
            if i == self.injection_layer:
                x = torch.cat((self.deep_prompt.unsqueeze(1).repeat(1, x.size(1), 1), x), dim=0)
